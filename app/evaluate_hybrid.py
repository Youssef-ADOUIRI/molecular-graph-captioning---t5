...
def compute_bleu4(predictions, references):
    ...
    bleu1 = corpus_bleu(
        ref_tokens, pred_tokens, weights=(1, 0, 0, 0), smoothing_function=smoother
    )
    bleu2 = corpus_bleu(
        ref_tokens, pred_tokens, weights=(0.5, 0.5, 0, 0), smoothing_function=smoother
    )
    bleu3 = corpus_bleu(
        ref_tokens,
        pred_tokens,
...
def main(args):
    ...
    train_embs = compute_graph_embeddings(
        train_graphs, gnn_encoder, device, args.batch_size
    )
    val_embs = compute_graph_embeddings(
        val_graphs, gnn_encoder, device, args.batch_size
    )
    ...
    print(
        f"Above threshold {args.threshold}: {(sims >= args.threshold).sum()} / {len(sims)}"
    )
    ...
    val_loader = DataLoader(
        val_graphs, batch_size=args.batch_size, shuffle=False, collate_fn=collate_graphs
    )
