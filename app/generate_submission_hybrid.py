...
def compute_graph_embeddings(graphs, encoder, device, batch_size=32):
    ...
    dataloader = DataLoader(
        graphs, batch_size=batch_size, shuffle=False, collate_fn=collate_graphs
    )
...
def main(args):
    ...
    reference_embs = compute_graph_embeddings(
        reference_graphs, gnn_encoder, device, args.batch_size
    )

    print("Computing test graph embeddings...")
    test_embs = compute_graph_embeddings(
        test_graphs, gnn_encoder, device, args.batch_size
    )
    ...
    print(
        f"Similarity stats: min={sims.min():.3f}, max={sims.max():.3f}, mean={sims.mean():.3f}"
    )
    print(
        f"Molecules above threshold {args.threshold}: {(sims >= args.threshold).sum()} / {len(sims)}"
    )
    ...
    # Create submission
    submission_df = pd.DataFrame({"ID": all_ids, "description": all_descriptions})
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Hybrid retrieval+generation submission"
    )
    ...
    parser.add_argument(
        "--subset", type=int, default=None, help="Subset size for debugging"
    )
