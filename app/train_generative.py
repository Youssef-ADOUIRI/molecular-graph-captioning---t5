...
from app.model import build_molecular_graph_model, Graph2CaptionModel
...
@torch.no_grad()
def validate(model: Graph2CaptionModel, dataloader: DataLoader, device: torch.device):
    """Compute validation loss."""
    model.eval()
    total_loss = 0.0
    num_batches = 0

    for graphs, input_ids, attention_mask, labels in tqdm(
...
def load_checkpoint(model, optimizer, path, device):
    ...
    if optimizer is not None:
        try:
            ...
            print(
                f"Warning: Could not load optimizer state (param count mismatch). Starting fresh optimizer."
            )
...
def main(args):
    ...
    val_ds = MolecularCaptionDataset(
        val_path, tokenizer_name=args.lm_name, max_length=args.max_length, is_test=False
    )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if (
            not args.use_lora
            and epoch == args.freeze_lm_epochs
            and args.freeze_lm_epochs > 0
        ):
        ...
        if (epoch + 1) % args.eval_every == 0:
            ...
            if val_loss < best_val_loss:
                ...
                save_checkpoint(
                    model, optimizer, epoch, val_loss, "checkpoints/best_model.pt"
                )

        # Periodic checkpoint
        if (epoch + 1) % args.save_every == 0:
            save_checkpoint(
                model, optimizer, epoch, train_loss, f"checkpoints/epoch_{epoch+1}.pt"
            )

    # Save final model
    save_checkpoint(
        model, optimizer, args.epochs, train_loss, "checkpoints/final_model.pt"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--weight_decay", type=float, default=DEFAULT_CONFIG["weight_decay"]
    )
    parser.add_argument("--max_length", type=int, default=DEFAULT_CONFIG["max_length"])
    parser.add_argument(
        "--freeze_lm_epochs", type=int, default=DEFAULT_CONFIG["freeze_lm_epochs"]
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--subset", type=int, default=None, help="Use subset for testing"
    )
    parser.add_argument(
        "--resume", type=str, default=None, help="Resume from checkpoint"
    )
    parser.add_argument(
        "--simple_bridge", action="store_true", help="Use simple bridge"
    )
    ...
    parser.add_argument(
        "--lora_r", type=int, default=DEFAULT_CONFIG["lora_r"], help="LoRA rank"
    )
    parser.add_argument(
        "--lora_alpha",
        type=int,
